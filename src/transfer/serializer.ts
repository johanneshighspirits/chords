...
export const serializeColor = ({ h, s, l }: Color): SerializedColor => [
  h,
  s,
  l,
];
type SerializedColor = [h: number, s: number, l: number];

export const deserializeColor = (data?: SerializedColor): Color => {
  if (!data) {
    return { h: 0, s: 0, l: 0 };
  }
  const [h, s, l] = data;
